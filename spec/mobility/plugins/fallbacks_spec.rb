...
describe Mobility::Plugins::Fallbacks do
  ...
  context "fallbacks is a hash" do
    ...
    it "returns value when value is not nil" do
      allow(listener).to receive(:read).once.with(:ja, {}).and_return([:ja, "ja val"])
      expect(backend.read(:ja)).to eq([:ja, "ja val"])
    end
    ...
    it "falls through to fallback locale when value is nil" do
      ...
      expect(backend.read(:'en-US')).to eq([:'de-DE', 'de val'])
    end
    ...
    it "returns backend value when no fallback is found" do
      ...
      expect(backend.read(:'en-US')).to eq([:'en-US', ''])
    end

    it "returns backend value when fallback: false option is passed" do
      expect(listener).to receive(:read).once.with(:'en-US', {}).and_return([:'en-US', ''])
      expect(backend.read(:'en-US', fallback: false)).to eq([:'en-US', ''])
    end
    ...
    it "falls through to fallback locale when fallback: true option is passed" do
      ...
      expect(backend.read(:'en-US', fallback: true)).to eq([:en, "en val"])
    end
    ...
    it "uses locale passed in as value of fallback option when present" do
      ...
      expect(backend.read(:'en-US', fallback: :ja)).to eq([:ja, "ja val"])
    end
    ...
    it "uses array of locales passed in as value of fallback options when present" do
      ...
      expect(backend.read(:"en-US", fallback: [:pl, :'de-DE'])).to eq([:pl, "pl val"])
    end

    it "passes options to getter in fallback locale" do
      expect(listener).to receive(:read).once.with(:'en-US', foo: true).and_return([:'en-US', "bar"])
      expect(backend.read(:'en-US', foo: true)).to eq([:'en-US', "bar"])
    end
  ...
  if ENV['I18N_FALLBACKS']
    context "fallbacks is true" do
      ...
      it "uses default fallbacks" do
        ...
        expect(backend.read(:'en-US')).to eq([:ja, 'ja val'])
  ...
  context "fallbacks is nil" do
    ...
    it "does not use fallbacks when accessor fallback option is false or nil" do
      expect(listener).to receive(:read).with(:'en-US', {}).once.and_return([:'en-US', ''])
      expect(backend.read(:'en-US')).to eq([:'en-US', ''])
      expect(listener).to receive(:read).with(:'en-US', {}).once.and_return([:'en-US', ''])
      expect(backend.read(:'en-US', fallback: false)).to eq([:'en-US', ''])
    end
    ...
    it "uses array of locales passed in as value of fallback options when present" do
      ...
      expect(backend.read(:'en-US', fallback: [:pl, :'de-DE'])).to eq([:pl, 'pl val'])
      expect(backend.read(:'en-US', fallback: [:'de-DE', :pl])).to eq([:'de-DE', 'de val'])
    end

    it "does not use fallbacks when fallback: true option is passed" do
      expect(listener).to receive(:read).once.with(:'en-US', {}).and_return([:'en-US', nil])
      expect(backend.read(:'en-US', fallback: true)).to eq([:'en-US', nil])
    end
  end
end
