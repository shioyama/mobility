  ...
  describe "when included into a class" do
    ...
    describe "#read" do
      ...
      it "does not cache reads with super: true option" do
        backend = cached_backend_class.new("model", "attribute")
        expect(backend.spy).to receive(:read).twice.with(locale, options.merge(super: true)).and_return("foo")
        2.times { expect(backend.read(locale, options.merge(super: true))).to eq("foo") }
      end
    ...
    describe "#write" do
      ...
      it "does not store value in cache with super: true option" do
        backend = cached_backend_class.new("model", "attribute")
        allow(backend.spy).to receive(:write).once.with(locale, "foo", options.merge(super: true)).and_return("bar")
        expect(backend.write(locale, "foo", options.merge(super: true))).to eq("bar")
        expect(backend.spy).to receive(:read).with(locale, options).and_return("baz")
        expect(backend.read(locale, options)).to eq("baz")
      end
