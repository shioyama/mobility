  ...
  context "with basic plugins" do
    ...
    describe "Backend methods" do
      ...
      describe "#read" do
        before do
          [
            { name: "title", body: "New Article", locale: "en", record: article },
            { name: "title", body: "新規記事", locale: "ja", record: article },
            { name: "content", body: "Once upon a time...", locale: "en", record: article },
            { name: "content", body: "昔々あるところに…", locale: "ja", record: article }
      ...
      describe "#write" do
        context "no translation for locale exists" do
          ...
          it "assigns attributes to translation" do
            ...
            aggregate_failures do
              expect(translation.name).to eq("title")
              expect(translation.body).to eq("New Article")
              expect(translation.record).to eq(subject)
            end
          end
        end
        ...
        context "translation for locale exists" do
          ...
          it "updates value attribute on existing translation" do
            ...
            aggregate_failures do
              expect(translation.name).to eq("title")
              expect(translation.body).to eq("New New Article")
              expect(translation.record).to eq(subject)
            end
          end
          ...
          it "removes persisted translation if assigned nil when record is saved" do
            ...
            expect(translation_class.count).to eq(0)
          end
    ...
    describe "translations association" do
      ...
      it "limits association to translations with keys matching attributes" do
        ...
        translation = translation_class.create(name: "foo", body: "bar", locale: "en", record: article)
        article = Article.first
    ...
    #       expect(text.body).to eq("foo title")
    #
    #       string = translation_class.second
    #       expect(string.body).to eq("bar short title")
    #     end
    #   end
    # end
    ...
    describe "after destroy" do
      ...
      before do
        ...
        translates Post, :title, :content, backend: :action_text
      end
      ...
      it "cleans up all associated translations, regardless of key" do
        ...
        expect(translation_class.count).to eq(6)

        translation_class.create!(record: article, name: "key1", body: "value1", locale: "de")
        translation_class.create!(record: article, name: "key2", body: "value2", locale: "fr")
        expect(translation_class.count).to eq(8)
